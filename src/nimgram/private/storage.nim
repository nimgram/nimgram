...
proc writeSessionsInfo*(self: NimgramStorageRam, dcOptions: Table[int,
        DcOption]) {.async.} =
    await writeBin(self.binFileName, dcOptions)

proc clearCache*(self: NimgramStorageRam) = self.peerStorage.clear()

proc getSessionsInfo*(self: NimgramStorageRam): Future[Table[int,
        DcOption]] {.async.} =
...
proc getPeer*(self: NimgramStorageRam, user: int64): Future[
        StoragePeer] {.async.} =
...
when compileOption("threads"):
    ...
    proc writeSessionsInfo*(self: NimgramStorageSqlite, dcOptions: Table[int,
            DcOption]) {.async.} =
        ...
        for _, key in dcOptions:
            ...
            await self.sqliteInstance.exec(sql(
                    &"INSERT INTO dcoptions (number, isAuthorized, isMain, authKey, salt) VALUES ({key.number}, {isAuthorized}, {isMain}, '{authKey}', '{salt}');"))

    proc addPeer*(self: NimgramStorageSqlite, user: StoragePeer) {.async.} =
        await self.sqliteInstance.exec(sql(
                &"INSERT INTO peerdata(id, access_hash) VALUES({user.peerID}, '{user.accessHash}') ON CONFLICT(id) DO UPDATE SET access_hash='{user.accessHash}';"))
        if not self.disableCache:
            #TODO: maybe add cache auto clean if we reach many chats stored?
            self.cache[user.peerID] = user

    proc getPeer*(self: NimgramStorageSqlite, user: int64): Future[
            StoragePeer] {.async.} =
        if not self.disableCache and self.cache.contains(user):
            return self.cache[user]
        var row = await self.sqliteInstance.getRow(sql(
                &"select access_hash from peerdata where id = {user}"))
        return StoragePeer(peerID: user, accessHash: row[0].parseBiggestInt)

    proc clearCache*(self: NimgramStorageSqlite) =
        self.cache.clear()

    proc getSessionsInfo*(self: NimgramStorageSqlite): Future[Table[int,
            DcOption]] {.async.} =
        ...
        for row in rows:
            ...
            result[int(number)] = DcOption(number: number,
                    isAuthorized: isAuthorized, isMain: isMain,
                    authKey: authKey, salt: salt)
...
proc writeSessionsInfo*(self: NimgramStorage, dcOptions: Table[int,
        DcOption]) {.async.} =
    if self of NimgramStorageRam:
...
proc getSessionsInfo*(self: NimgramStorage): Future[Table[int,
        DcOption]] {.async.} =
...
proc getPeer*(self: NimgramStorage, user: int64): Future[
        StoragePeer] {.async.} =
