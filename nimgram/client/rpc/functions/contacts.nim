...
method TLEncode*(self: ContactsGetTopPeers): seq[uint8] =
    ...
    result = result & TLEncode(self.limit)
    result = result & TLEncode(self.hash)
method TLDecode*(self: ContactsGetTopPeers, bytes: var ScalingSeq[uint8]) = 
...
method TLDecode*(self: ContactsAddContact, bytes: var ScalingSeq[uint8]) = 
    bytes.TLDecode(addr self.flags)
    if (self.flags and (1 shl 0)) != 0:
...
method TLDecode*(self: ContactsGetLocated, bytes: var ScalingSeq[uint8]) = 
    bytes.TLDecode(addr self.flags)
    if (self.flags and (1 shl 1)) != 0:
...
method TLDecode*(self: ContactsBlockFromReplies, bytes: var ScalingSeq[uint8]) = 
    bytes.TLDecode(addr self.flags)
    if (self.flags and (1 shl 0)) != 0:
