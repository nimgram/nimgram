...
proc TLEncode*(x: Int128, endiannes: Endianness = littleEndian): seq[uint8] =
    result = cast[array[0..15, uint8]](x)[0..15]
    if endiannes != cpuEndian:
...
proc TLEncode*(x: Int256, endiannes: Endianness = littleEndian): seq[uint8] =
    result = cast[array[0..31, uint8]](x)[0..31]
    if endiannes != cpuEndian:
...
proc emptyBytes(len: int): seq[uint8] =  
    for _ in countup(1, len): 
        result = result & 0
...
proc TLEncode*(data: seq[uint8], debug: bool = false): seq[uint8] =
   ...
   if lenght <= 253:
       ...
       #result = result & emptyBytes((lenght - 1) mod 4)
   else:
       ...
       while float32(len(result)) mod float32(4) != float32(0):
           result.add(uint8(0))
       #result = result & emptyBytes(lenght mod 4)
